    ...
    <NodeWrapper id={id} data={data} maxPerSide={3}
      ...
      {editing ? (
        <input autoFocus defaultValue={title}
          ...
          style={{ fontSize: 32, fontWeight: 700, color: '#747474', textAlign: 'center', background: 'transparent', outline: 'none', width: '100%', padding: '0 24px', border: 'none' }} />
      ) : (
        <div onDoubleClick={() => setEditing(true)}
          style={{ fontSize: 32, fontWeight: 700, color: '#747474', textAlign: 'center', cursor: 'text', padding: '0 24px', width: '100%' }}>
          {title}
        </div>
      )}
    </NodeWrapper>
