  ...
  return (
    <div ref={ref} style={{
      ...
      {nodeTypeList.map((opt) => (
        <div key={opt.type} onClick={() => { onSelect(opt.type); onClose() }}
          style={{ padding: '7px 16px', cursor: 'pointer', fontSize: 13, color: theme.text, display: 'flex', alignItems: 'center', gap: 10 }}
          onMouseEnter={(e) => e.currentTarget.style.background = theme.hoverBg}
          onMouseLeave={(e) => e.currentTarget.style.background = 'transparent'}>
          <opt.Icon size={15} color={theme.text} strokeWidth={1.8} />
          {opt.label}
        </div>
      ))}
    </div>
