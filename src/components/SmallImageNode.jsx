...
function SmallImageNode({ data, id }) {
  ...
  return (
    <NodeWrapper id={id} data={data} maxPerSide={3}
      style={{ width: 360, height: imgSize, border: `2px solid ${data.strokeColor || '#A99482'}`, borderRadius: 4.35, background: data.fillColor || 'white', display: 'flex', flexDirection: 'row', overflow: 'hidden' }}>
      {/* Square image area on left */}
      <div onClick={() => fileRef.current?.click()}
        style={{ width: imgSize, minWidth: imgSize, height: imgSize, background: '#DBD0C6', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', borderRight: '2px solid #A99482' }}>
